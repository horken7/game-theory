    ...
    for (var roundIdx = 0; roundIdx < nRounds; roundIdx++) {
      ...
      strategies.forEach(function (strategy, stratIdx) {
        ...
        try {
          ...
          // console.log(strategy.name, histories[stratIdx][roundIdx], action);
        }
        catch (e) {
          throw e;
        }
...
function playAllPairsSymmetric(game, strategies) {
  ...
  for (var i = 0; i < strategies.length; i++) {
    for (var j = 0; j <= i; j++) {
      scores = game([strategies[0], strategies[1]]);
...
function newAbundances(abundances, scoreMatrix, pressure, extinctionLimit) {
